    ...
    value: function create() {
      ...
      try {
        ...
        if ("withCredentials" in xhr) {
          ...
          console.log("this.opts.withCredentials",this.opts.withCredentials);
...
var Polling = /*#__PURE__*/function (_Transport) {
  ...
  _createClass(Polling, [{
    ...
    value: function onData(data) {
      var self = this;
      debug("gui polling got data %s", data);
      // var n = data.indexOf("{");
      // if (n > 0) {
      //   data = data.slice(n);
      //   debug("gui polling modified data %s", data);
      // }
      ...
      debug("gui sending  %s", data);
     
      let binaryType = this.socket.binaryType;
      debug("sending binaryType %s",binaryType);
      parser.decodePayload(data, binaryType).forEach(callback); // if an event did not trigger closing
...
var decodePayload = function decodePayload(encodedPayload, binaryType) {
  debug("recived data => %s",encodedPayload);
  debug("recived binaryType => %s",binaryType);
  var encodedPackets = encodedPayload.split(SEPARATOR);
  var packets = [];
  debug("recived binaryType => %s",JSON.stringify(encodedPackets));
  for (var i = 0; i < encodedPackets.length; i++) {
    var decodedPacket = decodePacket(encodedPackets[i], binaryType);
    debug("encoded %s , decodedPacket data => %s",encodedPackets[i],JSON.stringify(decodedPacket));
